...
function convert_wavelength_freq(
        wl_or_freq_input::R; input_unit="", output_unit="", decimals_to_round=nothing
    ) where R <: Real
    c = 299792458
    # speed of light
    ...
    elseif input_unit in ["cm^-1", "cm-1"]
        ...
        throw(DomainError("I don't know how to calculate for this `$input_unit` input unit"))
    end
    ...
    elseif output_unit in ["cm^-1", "cm-1"]
        ...
        throw(DomainError("I don't know how to calculate for this `$output_unit` output unit"))        
    end
...
function get_WaveVector(
        wl_or_freq_input;
        input_unit="",
        Eps = nothing, Mu = nothing,
        Eps_r = nothing, Mu_r = nothing
    )
    Omega = convert_wavelength_freq(
        wl_or_freq_input; input_unit=input_unit, output_unit="rad/s"
    )
